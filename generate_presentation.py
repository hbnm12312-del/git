...
# -*- coding: utf-8 -*-
"""
Clean presentation generator using the shared ppt_lib.

Usage:
    python generate_presentation.py
    python generate_presentation.py --texts texts.json --img-dir us_imgs_hd --out output.pptx
"""
import argparse, os, sys

# Ensure ppt_lib is importable from the same directory
...
from ppt_lib import (
    RED, GOLD, WHITE, OFF_W, LTGY, MDGY, DKBG, NAVY, BLUE,
    SW, SH, LI,
    ap, BG, Tb, Ac, Btm, Top, Hd, Pn, Cd,
    new_presentation, load_texts,
)


def generate(prs, T, img_dir):
    """Build all 10 slides.  T is the dict from texts.json."""

    # ---- Slide 1: Title -----------------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 0); Top(s); Btm(s)
    ...
    Tb(s, Emu(500000), Emu(500000), Emu(3800000), Emu(500000),
       T["s2_title_cn"], 36, True, WHITE, PP_ALIGN.CENTER)
    Tb(s, Emu(500000), Emu(950000), Emu(3800000), Emu(300000),
       T["s2_title_en"], 16, False, GOLD, PP_ALIGN.CENTER)
    ...
    Tb(s, cx, Emu(2000000), circ, Emu(400000),
       T.get("s2_name", ""), 26, True, WHITE, PP_ALIGN.CENTER)
    ...
    Tb(s, Emu(500000), Emu(3650000), Emu(3800000), Emu(200000),
       T["s2_edu_title"], 13, True, GOLD, PP_ALIGN.CENTER)
    edu = [T["s2_edu_1"], T["s2_edu_2"], T["s2_edu_3"]]
    for i, e in enumerate(edu):
        Tb(s, Emu(500000), Emu(3900000 + i * 250000), Emu(3800000), Emu(220000),
           "\u25b8  " + e, 11, False, OFF_W)
    rx = Emu(4800000); rw = Emu(int(SW - 5200000))
    Tb(s, rx, Emu(500000), rw, Emu(400000),
       T["s2_career_title_cn"], 30, True, WHITE)
    Tb(s, rx, Emu(880000), rw, Emu(250000),
       T["s2_career_title_en"], 14, False, GOLD)
    ...
    career = [
        ("s2_c1", T["s2_c1_per"], T["s2_c1_tit"], T["s2_c1_desc"]),
    ...
    for i, (k, per, tit, desc) in enumerate(career):
        ...
        Tb(s, tlx + Emu(250000), yb - Emu(50000), rw - Emu(250000), Emu(250000),
           per, 10, False, GOLD)
        Tb(s, tlx + Emu(250000), yb + Emu(200000), rw - Emu(250000), Emu(280000),
           tit, 18, True, WHITE)
        Tb(s, tlx + Emu(250000), yb + Emu(480000), rw - Emu(250000), Emu(400000),
           desc, 11, False, LTGY)
    Btm(s)
    print("Slide 2 done")
    ...
    # ---- Slide 3: Vision ----------------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 2); Btm(s)
    Hd(s, T["s3_title_cn"], T["s3_title_en"])
    visions = [
        ("\u2605", T["s3_v1_cn"], T["s3_v1_en"], T["s3_v1_desc"]),
    ...
    for i, (star, cn, en, desc) in enumerate(visions):
        ...
        Tb(s, cx + Emu(300000), cy + Emu(1700000), cw - Emu(600000), Emu(1500000),
           desc, 14, False, LTGY, PP_ALIGN.CENTER)
    Tb(s, Emu(1500000), int(SH - 900000), Emu(int(SW - 3000000)), Emu(350000),
       "\u201c" + T["s3_quote"] + "\u201d", 15, False, GOLD, PP_ALIGN.CENTER)
    Tb(s, Emu(1500000), int(SH - 600000), Emu(int(SW - 3000000)), Emu(250000),
       "\u201c" + T["s3_quote_en"] + "\u201d", 12, False, LTGY, PP_ALIGN.CENTER)
    print("Slide 3 done")

    # ---- Slide 4: Economy ---------------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 3); Btm(s)
    Hd(s, T["s4_title_cn"], T["s4_title_en"])
    Pn(s, Emu(400000), Emu(1500000), Emu(6200000), int(SH - 2400000))
    Tb(s, Emu(600000), Emu(1700000), Emu(5800000), Emu(350000),
       "\u261b  " + T["s4_sub"], 18, True, GOLD)
    pols = [
        ("01", T["s4_p1_cn"], T["s4_p1_en"], T["s4_p1_desc"]),
    ...
    for i, (num, cn, en, desc) in enumerate(pols):
        ...
        Tb(s, Emu(700000), yb + Emu(70000), Emu(500000), Emu(380000),
           num, 16, True, WHITE, PP_ALIGN.CENTER)
        Tb(s, Emu(1350000), yb + Emu(20000), Emu(5000000), Emu(260000),
           cn + "  /  " + en, 16, True, WHITE)
        Tb(s, Emu(1350000), yb + Emu(280000), Emu(5000000), Emu(250000),
           desc, 12, False, LTGY)
    r2x = Emu(6900000); r2w = Emu(int(SW - 7400000))
    Pn(s, r2x, Emu(1500000), r2w, int(SH - 2400000))
    Tb(s, r2x + Emu(200000), Emu(1700000), r2w - Emu(400000), Emu(250000),
       T["s4_goal"], 16, True, GOLD, PP_ALIGN.CENTER)
    Ac(s, int(r2x + r2w / 2 - 300000), Emu(2000000), Emu(600000), RED, Emu(3000))
    for i, (num, lab) in enumerate([
        (T["s4_s1_num"], T["s4_s1_lab"]),
        (T["s4_s2_num"], T["s4_s2_lab"]),
        (T["s4_s3_num"], T["s4_s3_lab"]),
    ]):
        ys = Emu(2300000 + i * 1200000)
        Tb(s, r2x + Emu(200000), ys, r2w - Emu(400000), Emu(350000),
           num, 36, True, GOLD, PP_ALIGN.CENTER)
        Tb(s, r2x + Emu(200000), ys + Emu(380000), r2w - Emu(400000), Emu(250000),
           lab, 13, True, WHITE, PP_ALIGN.CENTER)
    print("Slide 4 done")

    # ---- Slide 5: Immigration -----------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 4); Btm(s)
    Hd(s, T["s5_title_cn"], T["s5_title_en"])
    Pn(s, Emu(400000), Emu(1500000), int(SW - 800000), int(SH - 2400000))
    col_w = Emu(int((SW - 1800000) / 2))
    Tb(s, Emu(600000), Emu(1700000), col_w, Emu(300000),
       "\u26d4  " + T["s5_left_title"], 18, True, RED)
    left = [T["s5_l1"], T["s5_l2"], T["s5_l3"], T["s5_l4"], T["s5_l5"]]
    for i, it in enumerate(left):
        Tb(s, Emu(750000), Emu(2100000 + i * 320000), col_w + Emu(500000), Emu(280000),
           "\u2716  " + it, 12, False, OFF_W)
    rx2 = Emu(6600000)
    Tb(s, rx2, Emu(1700000), col_w, Emu(300000),
       "\u2714\ufe0f  " + T["s5_right_title"], 18, True, GOLD)
    right = [T["s5_r1"], T["s5_r2"], T["s5_r3"], T["s5_r4"], T["s5_r5"]]
    for i, it in enumerate(right):
        Tb(s, rx2 + Emu(100000), Emu(2100000 + i * 320000), col_w + Emu(500000), Emu(280000),
           "\u2714  " + it, 12, False, OFF_W)
    div = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, int(SW / 2 - 2000), Emu(2100000),
                             Emu(4000), Emu(1600000))
    ...
    Tb(s, Emu(800000), int(SH - 800000), Emu(int(SW - 1600000)), Emu(300000),
       "\u201c" + T["s5_quote"] + "\u201d", 14, False, GOLD, PP_ALIGN.CENTER)
    Tb(s, Emu(800000), int(SH - 550000), Emu(int(SW - 1600000)), Emu(250000),
       "\u201c" + T["s5_quote_en"] + "\u201d", 11, False, LTGY, PP_ALIGN.CENTER)
    print("Slide 5 done")

    # ---- Slide 6: Foreign Policy --------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 5); Btm(s)
    ...
    Tb(s, Emu(500000), Emu(1700000), int(SW - 1200000), Emu(300000),
       "\u261b  " + T["s6_sub"], 18, True, GOLD)
    pillars = [
        ("\U0001f30d", T["s6_p1_cn"], T["s6_p1_en"], T["s6_p1_desc"], T["s6_p1_en_desc"]),
        ("\U0001f30e", T["s6_p2_cn"], T["s6_p2_en"], T["s6_p2_desc"], T["s6_p2_en_desc"]),
        ("\U0001f30f", T["s6_p3_cn"], T["s6_p3_en"], T["s6_p3_desc"], T["s6_p3_en_desc"]),
    ]
    pw = Emu(3600000); ph = Emu(2800000); pgap = Emu(250000); psx = Emu(int(450000))
    for i, (icon, cn, en, cnd, end) in enumerate(pillars):
        ...
        Tb(s, px + Emu(200000), Emu(3450000), pw - Emu(400000), Emu(1300000),
           cnd, 12, False, OFF_W)
        Tb(s, px + Emu(200000), Emu(4600000), pw - Emu(400000), Emu(350000),
           end, 10, False, MDGY)
    print("Slide 6 done")

    # ---- Slide 7: Infrastructure & Healthcare -------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 6); Btm(s)
    ...
    Tb(s, Emu(600000), Emu(1650000), Emu(5000000), Emu(300000),
       "\U0001f3d7\ufe0f  " + T["s7_infra_title"], 18, True, GOLD)
    infra = [T["s7_infra_1"], T["s7_infra_2"], T["s7_infra_3"], T["s7_infra_4"]]
    for i, it in enumerate(infra):
        Tb(s, Emu(700000), Emu(2050000 + i * 280000), Emu(5500000), Emu(250000),
           "\u25b8  " + it, 12, False, OFF_W)
    rhx = Emu(6500000)
    Tb(s, rhx, Emu(1650000), Emu(5000000), Emu(300000),
       "\U0001f3e5  " + T["s7_health_title"], 18, True, RED)
    health = [T["s7_health_1"], T["s7_health_2"], T["s7_health_3"], T["s7_health_4"]]
    for i, it in enumerate(health):
        Tb(s, rhx + Emu(100000), Emu(2050000 + i * 280000), Emu(5500000), Emu(250000),
           "\u25b8  " + it, 12, False, OFF_W)
    div2 = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, int(SW / 2 - 2000), Emu(2000000),
                              Emu(4000), Emu(1200000))
    div2.fill.solid(); div2.fill.fore_color.rgb = GOLD; div2.line.fill.background()
    for i, (num, lab) in enumerate([
        (T["s7_s1_num"], T["s7_s1_lab"]),
        (T["s7_s2_num"], T["s7_s2_lab"]),
        (T["s7_s3_num"], T["s7_s3_lab"]),
    ]):
        sx2 = Emu(1200000 + i * 3300000)
        Tb(s, sx2, Emu(3400000), Emu(2800000), Emu(350000),
           num, 32, True, GOLD, PP_ALIGN.CENTER)
        Tb(s, sx2, Emu(3750000), Emu(2800000), Emu(250000),
           lab, 13, False, LTGY, PP_ALIGN.CENTER)
    print("Slide 7 done")

    # ---- Slide 8: Education -------------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 7); Btm(s)
    Hd(s, T["s8_title_cn"], T["s8_title_en"])
    Pn(s, Emu(400000), Emu(1500000), int(SW - 800000), int(SH - 2400000))
    Tb(s, Emu(600000), Emu(1700000), int(SW - 1200000), Emu(300000),
       "\u261b  " + T["s8_sub"], 18, True, GOLD)
    reforms = [
        ("\U0001f393", T["s8_r1_cn"], T["s8_r1_en"], T["s8_r1_desc"]),
    ...
    for i, (icon, cn, en, desc) in enumerate(reforms):
        col = i % 2; row = i // 2
        ix = Emu(600000) + col * Emu(5600000)
        iy = Emu(2150000) + row * Emu(1450000)
        Tb(s, ix, iy, Emu(500000), Emu(400000), icon, 24, False, WHITE)
        Tb(s, ix + Emu(500000), iy, Emu(5000000), Emu(300000),
           cn + "  /  " + en, 16, True, WHITE)
        Tb(s, ix + Emu(500000), iy + Emu(350000), Emu(5000000), Emu(400000),
           desc, 11, False, OFF_W)
    print("Slide 8 done")

    # ---- Slide 9: Why Vote --------------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 8); Btm(s)
    Hd(s, T["s9_title_cn"], T["s9_title_en"])
    Pn(s, Emu(400000), Emu(1500000), int(SW - 800000), int(SH - 2400000))
    reasons = [
        ("\U0001f3db\ufe0f", T["s9_r1_cn"], T["s9_r1_en"], T["s9_r1_desc"]),
    ...
    rw2 = Emu(2700000); rh2 = Emu(2100000); rgap = Emu(250000); rsx = Emu(550000)
    for i, (icon, cn, en, desc) in enumerate(reasons):
        col = i % 2; row = i // 2
        rx3 = rsx + col * (rw2 + rgap + Emu(100000))
        ry3 = Emu(1750000) + row * (rh2 + Emu(150000))
        Cd(s, rx3, ry3, rw2, rh2)
        Tb(s, rx3, ry3 + Emu(150000), rw2, Emu(350000), icon, 24, False, WHITE, PP_ALIGN.CENTER)
        Tb(s, rx3, ry3 + Emu(500000), rw2, Emu(300000), cn, 20, True, WHITE, PP_ALIGN.CENTER)
        Tb(s, rx3, ry3 + Emu(800000), rw2, Emu(250000), en, 12, False, GOLD, PP_ALIGN.CENTER)
        Ac(s, int(rx3 + rw2 / 2 - 250000), ry3 + Emu(1100000), Emu(500000), GOLD, Emu(3000))
        Tb(s, rx3 + Emu(150000), ry3 + Emu(1200000), rw2 - Emu(300000), Emu(700000),
           desc, 12, False, LTGY, PP_ALIGN.CENTER)
    print("Slide 9 done")

    # ---- Slide 10: Call to Action -------------------------------------
    s = prs.slides.add_slide(prs.slide_layouts[LI])
    BG(s, img_dir, 9); Top(s); Btm(s)
    Ac(s, 0, Emu(100000), SW, GOLD, Emu(2000))
    Ac(s, 0, int(SH - 100000), SW, GOLD, Emu(2000))
    Tb(s, Emu(500000), Emu(1200000), Emu(int(SW - 1000000)), Emu(600000),
       T["s10_slogan"], 48, True, WHITE, PP_ALIGN.CENTER)
    Tb(s, Emu(500000), Emu(1750000), Emu(int(SW - 1000000)), Emu(350000),
       T["s10_slogan_en"], 36, True, GOLD, PP_ALIGN.CENTER)
    stars2 = "\u2605 \u00b7 \u2605 \u00b7 \u2605 \u00b7 \u2605 \u00b7 \u2605"
    Tb(s, Emu(500000), Emu(2300000), Emu(int(SW - 1000000)), Emu(300000),
       stars2, 18, False, RED, PP_ALIGN.CENTER)
    Tb(s, Emu(500000), Emu(2700000), Emu(int(SW - 1000000)), Emu(350000),
       T["s10_vote_cn"], 28, True, WHITE, PP_ALIGN.CENTER)
    Tb(s, Emu(500000), Emu(3050000), Emu(int(SW - 1000000)), Emu(300000),
       T["s10_vote_en"], 18, True, GOLD, PP_ALIGN.CENTER)
    ...
    Tb(s, Emu(int(SW / 2 - 2300000)), Emu(3680000), Emu(4600000), Emu(260000),
       "\u201c" + T["s10_motto"] + "\u201d", 16, True, WHITE, PP_ALIGN.CENTER)
    Tb(s, Emu(int(SW / 2 - 2300000)), Emu(3950000), Emu(4600000), Emu(250000),
       "\u201c" + T["s10_motto_en"] + "\u201d", 12, False, GOLD, PP_ALIGN.CENTER)
    Tb(s, Emu(500000), Emu(4650000), Emu(int(SW - 1000000)), Emu(250000),
       T["s10_footer_cn"], 13, False, LTGY, PP_ALIGN.CENTER)
    Tb(s, Emu(500000), Emu(4880000), Emu(int(SW - 1000000)), Emu(250000),
       T["s10_footer_en"], 11, False, MDGY, PP_ALIGN.CENTER)
    print("Slide 10 done")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a 10-slide presentation from texts.json."
    )
    parser.add_argument(
        "--texts", default="texts.json",
        help="Path to texts.json (default: texts.json)"
    )
    parser.add_argument(
        "--img-dir", default="us_imgs_hd",
        help="Directory containing .jpg background images"
    )
    parser.add_argument(
        "--out", default="Ding_Dongxu_President.pptx",
        help="Output .pptx path"
    )
    args = parser.parse_args()

    texts_path = os.path.abspath(args.texts)
    ...
    T = load_texts(texts_path)
    print(f"Loaded {len(T)} text keys from {texts_path}")

    prs = new_presentation()
    generate(prs, T, img_dir)
